...
var KisBpmformWithVarCtrl = [ '$scope', '$modal', '$timeout', '$translate', function($scope, $modal, $timeout, $translate) {
...
var KisBpmformWithVarPopupCtrl = ['$scope', '$q', '$translate', function($scope, $q, $translate) {

    // Put json representing form properties on scope
    if ($scope.property.value !== undefined && $scope.property.value !== null
        && $scope.property.value.formWithVar !== undefined
        && $scope.property.value.formWithVar !== null) {
        // Note that we clone the json object rather then setting it directly,
        // this to cope with the fact that the user can click the cancel button and no changes should have happended
        //设置属性
        $scope.formWithVar = angular.copy($scope.property.value.formWithVar);
    } else {
    ...
    $scope.addNewProperty = function() {
        $scope.formWithVar.push({ id : 'new_property_' + propertyIndex++,
            name : '',
            type : 'string',
            readable: true,
            writable: true});
    ...
    $scope.removeProperty = function() {
        if ($scope.selectedProperties.length > 0) {
            var index = $scope.formWithVar.indexOf($scope.selectedProperties[0]);
            $scope.gridOptions.selectItem(index, false);
            $scope.formWithVar.splice(index, 1);

            $scope.selectedProperties.length = 0;
            if (index < $scope.formWithVar.length) {
                $scope.gridOptions.selectItem(index + 1, true);
            } else if ($scope.formWithVar.length > 0) {
                $scope.gridOptions.selectItem(index - 1, true);
            }
        }
    };
    ...
    $scope.movePropertyUp = function() {
        if ($scope.selectedProperties.length > 0) {
            ...
            if (index != 0) { // If it's the first, no moving up of course
                ...
                var temp = $scope.formWithVar[index];
                $scope.formWithVar.splice(index, 1);
                $timeout(function(){
                    $scope.formWithVar.splice(index + -1, 0, temp);
                }, 100);
    ...
    $scope.movePropertyDown = function() {
        if ($scope.selectedProperties.length > 0) {
            var index = $scope.formWithVar.indexOf($scope.selectedProperties[0]);
            if (index != $scope.formWithVar.length - 1) { // If it's the last element, no moving down of course
                // Reason for funny way of swapping, see https://github.com/angular-ui/ng-grid/issues/272
                var temp = $scope.formWithVar[index];
                $scope.formWithVar.splice(index, 1);
                $timeout(function(){
    ...
    $scope.save = function() {
        ...
        if ($scope.formWithVar.length > 0) {
            $scope.property.value = {};
            $scope.property.value.formWithVar = $scope.formWithVar;
        } else {
